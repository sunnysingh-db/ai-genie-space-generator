...
class GenieSpaceFramework:
    ...
    def __init__(self, config_yaml: str = None, config_path: str = None):
        ...
        # Table list support
        self.table_list = self.config_handler.get_table_list()
        ...
        print("=" * 80)
        if self.use_table_list:
            print(f"Mode: Table List ({len(self.table_list)} tables)")
            for fq in self.table_list[:10]:
                print(f"  - {fq}")
            if len(self.table_list) > 10:
                print(f"  ... and {len(self.table_list) - 10} more")
            print(f"Target Schema: {self.catalog}.{self.schema} (for metric views)")
        else:
            print(f"Mode: Full Schema Scan")
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Inject table_fq_map into llm_config for downstream components
            if self.table_fq_map:
                llm_config['table_fq_map'] = self.table_fq_map
            ...
            print("=" * 80)
            view_generator = MetricViewGenerator(
                self.spark, self.catalog, self.schema,
                table_fq_map=self.table_fq_map if self.use_table_list else None
            )
            ...
            print("=" * 80)
            genie_creator = GenieSpaceCreator(
